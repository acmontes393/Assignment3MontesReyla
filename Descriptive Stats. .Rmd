...
Our dependent variable is the total migrant stock at mid-year by country of origin, which we retrieved from the United Nations Population division. It collected census data from 232 countries in 1990, 2000, 2010 and 2013. 

The Figure 1 below shows the distribution of the dependent variable. Since the statistics accounts for the number of emigrants, it can be considered as count data. It is also very rightly skewed, because there is a large amount countries that both have low emigration and low population. 
...
Figure 2 expresses emigration in per capita terms in order to account for population differences between countries. The shape of the distribution is not as not made a visual change, which means that emigration is quite low in a large number of countries. 
